...
dat_stan_C.Ma <- list(
  ...
  p = df_hetero_simul[,8:23]/rowSums(df_hetero_simul[,8:24])
)
...
dat_stan_N.Ma <- list(
  ...
  litterBM = rowSums(df_hetero_simul[,8:23]),
  litterSR = rowSums(df_hetero_simul[,8:23]!=0),
  p = df_hetero_simul[,8:23]/rowSums(df_hetero_simul[,8:24]) # each row = 1 observations 
)
